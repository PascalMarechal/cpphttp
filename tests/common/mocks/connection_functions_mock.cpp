...
{
    createBufferFunction();
    ON_CALL(*this, async_read_header).WillByDefault([this](SocketMockWrapper &socket, std::string &buffer, cpphttp::internal::match_end_of_header &matcher, std::function<void(std::error_code, std::size_t)> callback) {
...
void ConnectionFunctionsMock::createErrorInBodyRead()
{
    ...
    ON_CALL(*this, async_read_body).WillByDefault([this](SocketMockWrapper &socket, std::string &buffer, asio::detail::transfer_exactly_t end, std::function<void(std::error_code, std::size_t)> callback) {
        callback(std::make_error_code(std::errc::io_error), 0);
    });
}
...
{
    ON_CALL(*this, async_read_header).WillByDefault([this, loops, dataToRead](SocketMockWrapper &socket, std::string &buffer, cpphttp::internal::match_end_of_header &matcher, std::function<void(std::error_code, std::size_t)> callback) {
...
{
    ON_CALL(*this, async_read_body).WillByDefault([this, loops, dataToRead](SocketMockWrapper &socket, std::string &buffer, asio::detail::transfer_exactly_t end, std::function<void(std::error_code, std::size_t)> callback) {
